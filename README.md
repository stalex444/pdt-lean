...
> **283 declarations · 19 modules · no `sorry` outside the Palomar comparator's `Challenge.lean` stubs (which state 13 compared theorems, all proved in `Solution.lean`) · no `native_decide`.** A mix of substantive results, API wrappers, and disclosed elementary numeral identities — the per-module table below states what each module proves; all kernel-clean over the three standard axioms (`propext`, `Classical.choice`, `Quot.sound`). (196 use the `theorem` keyword; the remaining 87 are `lemma` helpers. Six of the 283 are `private` and so are not addressable by name from an importing file: the audit checks the other 277 directly, and those six transitively, since `#print axioms` reports the axioms of everything a result depends on.) The one-command verifier below builds the project and prints **PASS** with the full axiom trace — or fails loudly.
...
| **PdtTsirelsonNorm / PdtTsirelsonExact** | The **exact CHSH constants**: the saturating tuple's l2 operator norm is **exactly 2√2** via the C\*-identity (and its square is provably *not* the scalar `8·1` — the textbook shortcut is false); the **Landau identity** `T² = 4 + [A₀,A₁][B₁,B₀]` in any \*-ring; `‖T‖ ≤ 2√2` in every nontrivial unital real C\*-normed algebra; a commuting party forces `‖T‖ = 2` **exactly**; any violation `‖T‖ > 2` forces noncommutativity in **both** parties; and `2√2` is the **greatest** CHSH operator norm over `M₄(ℝ)` (`IsGreatest`, attained). |
