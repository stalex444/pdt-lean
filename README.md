...
> **830 declarations · 36 modules · no `sorry` outside the Palomar comparator's `Challenge.lean` stubs (which state 2 compared theorems, all proved in `Solution.lean`) · no `native_decide`.** A mix of substantive results, API wrappers, and disclosed elementary numeral identities — the per-module table below states what each module proves; all kernel-clean over the three standard axioms (`propext`, `Classical.choice`, `Quot.sound`). (543 use the `theorem` keyword; the remaining 287 are `lemma` helpers. Six of the 830 are `private` and so are not addressable by name from an importing file: the audit checks the other 824 directly, and those six transitively, since `#print axioms` reports the axioms of everything a result depends on.) The one-command verifier below builds the project and prints **PASS** with the full axiom trace — or fails loudly.
...
| **PdtTraceLink** | **The hand-written integer matrices really are the trace-form Gram matrices.** `PdtSignature` and `PdtSignatureRho` introduce `M` (det −283) and `Mρ` (det −23) by hand, with Newton's identities quoted in a comment. This file *proves* the link through the genuine Mathlib API: `M` and `Mρ` are the Gram matrices of `(x, y) ↦ Tr_{K/ℚ}(x y)` in the power basis of `K = ℚ[X]/(f)` for the quartic resp. the cubic, and their determinants are the power-basis `Algebra.discr`. Built on `AdjoinRoot`, `PowerBasis` and `Algebra.traceMatrix`. The two polynomials are taken from `PdtNorm` rather than re-declared, so this module and `PdtNorm` can be imported together. |
| **PdtTraceSignature** | **The signatures as basis-free invariants.** Upgrades the matrix-level congruence certificates to statements about the quadratic form itself, using Mathlib's `sigPos` / `sigNeg` (the maximal dimension of a positive- resp. negative-definite subspace) and the uniqueness half of Sylvester's law (`QuadraticForm.sigPos_of_equiv_weightedSumSquares`): the trace form of `ℚ[x]/(x⁴ − x − 1)` has signature **(3, 1)** (`sigPos_traceQ`, `sigNeg_traceQ`) and that of `ℚ[x]/(x³ − x − 1)` has **(2, 1)** (`sigPos_traceρ`, `sigNeg_traceρ`). Intrinsic to the field: independent of the basis and of the diagonalising congruence. |
| **PdtTraceCompositum** | **The compositum `ℚ(ρ, Q)` and its trace form.** For `L = ℚ[x]/(x³−x−1) ⊗_ℚ ℚ[x]/(x⁴−x−1)`, a commutative ℚ-algebra of dimension 12: `trace_tmul` (`Tr_L(x ⊗ y) = Tr_k x · Tr_K y`), `traceForm_tmul`, the Gram matrix in the tensor basis as the **Kronecker product** `Mρ ⊗ₖ M` (`traceMatrix_bL`), the discriminant **(−23)⁴ · (−283)³** (`discr_bL`), and the Sylvester signature **(7, 5)** (`sigPos_traceL`, `sigNeg_traceL`) — obtained from the tensor of the two diagonalising bases, with `2·3 + 1·1 = 7` positive and `2·1 + 1·3 = 5` negative weights. `L_isField`: the tensor product is a field (coprime degrees). |
| **PdtTraceTensor** | **The same facts in general form**, for arbitrary finite free algebras `A`, `B` over a commutative ring and arbitrary quadratic forms over a linearly ordered field: `trace_tmul_general`, `traceForm_tmul_general`, `traceMatrix_tensorProduct` (Kronecker), `discr_tensorProduct` (`disc(bA ⊗ bB) = disc(bA)^{\|κ\|} · disc(bB)^{\|ι\|}`), `qTrace_tensor`, and the **Sylvester signature product rule** `sigPos/sigNeg (Q₁ ⊗ Q₂) = (p₁p₂ + n₁n₂, p₁n₂ + n₁p₂)` (`sigPos_tmul`, `sigNeg_tmul`). The compositum results above are re-derived as instances. |
| **PdtSignatureForcing** | **A square root of −1 obstructs a positive-definite associative form.** For any *associative* bilinear form `B` on a commutative algebra (`B (x*z) y = B x (z*y)`) and any `j` with `j² = −1`, `B j j = −B 1 1` (`assoc_sq_eq_neg_one`); over ℝ on a nontrivial algebra no such form is positive definite (`no_posDef_assoc_of_sq_neg_one`), and every trace form is associative (`traceForm_assoc`). At a complex place the associative forms are `T_λ(z,w) = Tr_{ℂ/ℝ}(λzw)`, whose Gram block in `{1, i}` has determinant **exactly −4‖λ‖² for every twist λ** (`lamForm_block_det`, `lamForm_block_det_neg`), with an explicit negative direction in each case. That is the **local** half of "negative index ≥ r₂"; transporting it to the field would need `K ⊗_ℚ ℝ ≅ ℝ^{r₁} × ℂ^{r₂}`, which is **not** formalized here. Two instances through the genuine `Algebra.traceForm`, each packaged with its Sylvester certificate: the quartic with timelike vector `4·root − 3·root²`, `Tr(x²) = −36` (`lorentzian_signature_of_pdt_quartic`), and the cubic with `4 + 9ρ − 6ρ²`, `Tr(x²) = −138` (`lorentzian_signature_of_pdt_cubic`) — the cubic trace form is constructed here and identified with `PdtSignatureRho`'s matrix (`ctraceForm_eq_Mrho`). A **CM control**: `ℚ(ζ₅)`'s power-basis trace form is (2,2), while its conjugation-twisted form is positive definite — a CM field parts company on the *twist*, not on the trace form. (That those two matrices are `ℚ(ζ₅)`'s Gram matrices is asserted, not proved here; the linear algebra about them is.) |
| **PdtTimeStepTrace** | **Multiplication by the quartic root does not preserve the trace form's timelike set.** With the witness `tW = 4·r − 3·r²` of the genuine quartic trace form: `traceForm tW tW = −36` and `traceForm (r·tW) (r·tW) = 91` (`traceForm_mul_tW`), hence `timelike_to_spacelike` (the sign flip) and `not_preserves_timelike` (the universally quantified no-go). A statement about one specific linear action on one specific quadratic form; it makes no physical identification and says nothing about other event maps. |
| **PdtGalois** | **Galois groups of the two polynomials.** `galActionHom_bijective_of_quartic` — a general criterion: an irreducible monic quartic over ℚ with exactly two non-real roots and an irreducible resolvent cubic has Galois group `S₄`. Then `gal_cubic_bijective` (`Gal(x³ − x − 1) ≅ S₃`, via Mathlib's prime-degree criterion) and `gal_quartic_bijective` (`Gal(x⁴ − x − 1) ≅ S₄`, via the criterion above with resolvent cubic `x³ + 4x − 1`, irreducible mod 7). |
| **PdtEhrenfest** | **Ehrenfest's 1917 dimensionality argument, as real analysis.** For the effective potential `U(r) = a r⁻² − b r⁻ᵖ` (`a, b > 0`, real exponent `p` via `Real.rpow`; Newtonian `n` dimensions is `p = n − 2`): the critical-point criterion and its uniqueness for `p ≠ 2`; `U''(rstar) = 2a(2 − p) rstar⁻⁴`, whose sign is the sign of `2 − p`; stability for `p < 2` and instability for `p > 2`; **no local minimum at all** for `p > 2`, hence no bounded non-circular motion; and `stable_iff_three` — among `n ≥ 3`, and for `a ≠ b`, a local minimum exists **iff `n = 3`**. Also the logarithmic `n = 2` case and the frequency ratio `U''(rstar)·rstar⁴/(2a) = 2 − p = 4 − n`. Every statement is about real functions of one real variable; the physical reading is not formalized. |
| **PdtLuders** | **The Lüders measurement-update rule, two classical theorems, finite dimension over `Matrix n n ℂ`.** Generic quantum mechanics — no arithmetic datum appears in any statement or proof. `ludersMap_fixed_iff_commute`: the **unsharp non-disturbance theorem** in square-root form — for Hermitian `A i` with `∑ A i * A i = 1`, the map `B ↦ ∑ A i * B * A i` fixes `B` iff every `A i` commutes with `B`. `luders_fixed_iff_commute_effects`: the same in the effects form of Arias–Gheondea–Gudder Thm 3.5(a) — in finite dimension the fixed-point set of a Lüders operation is exactly the commutant of the effects. `coherence_forces_luders` and `exists_coherent_update`: the coherence characterization (Fiorentino–Weigert Thm 1). **Scope riders, binding and stated in the file:** these are *fixed-point* and *characterization* theorems for **non-composite** systems; nothing here licenses treating the Lüders map as the only admissible update rule for composite systems, which is a separate theorem. |
| **PdtHamiltonianBoundary** | **The Hamiltonian boundary of one complex arithmetic tick.** A nonzero complex multiplier splits into a positive radial factor and a unit-modulus phase; the phase is generated by exactly the real scalar Hamiltonians `h_k = (2πk − arg σ)/Δt`, `k ∈ ℤ`, the whole logarithm-branch ambiguity classified (`scalarEvolution_eq_phase_iff`), while a raw multiplier whose norm differs from one is generated by **no** real scalar Hamiltonian. The cubic and quartic conjugate bounds then put the two roots on opposite sides of the radial boundary with both raw ticks outside scalar Hermitian evolution. Mathematical statements about one complex mode: they choose no physical tick duration, branch, observable algebra or open-system dynamics. |
| **PdtSphereSurface** | **The closed-form total surface measure of the Euclidean unit sphere.** Mathlib already carries the polar surface measure `Measure.toSphere` and the relation `surface(Sⁿ⁻¹) = n · vol(Bⁿ)`; what is missing is the closed-form evaluation for `EuclideanSpace ℝ ι`, parallel to `EuclideanSpace.volume_ball`. `EuclideanSpace.volume_toSphere_univ` supplies it — `2·√π^n / Γ(n/2)`, i.e. `2π^{n/2}/Γ(n/2)` — and `volume_toSphere_univ_fin_four` gives the case `n = 4`: the surface of `S³ ⊆ ℝ⁴` is **2π²**, four times `vol(B⁴) = π²/2`. A candidate contribution back to Mathlib. |
...
| PALOMAR-2026-09-01-000012 | [golden-family-boundary](https://github.com/stalex444/golden-family-boundary) | the golden family x^n = x + 1: one shared lemma (no root at a primitive cube root of unity), the morphic boundary iff n ≤ 3 unconditionally, the measure/Pisot equivalence, the Siegel-conditional excess, and the degree-4 separation — eleven theorems |
| PALOMAR-2026-09-02-000003 | [salem-theorem](https://github.com/stalex444/salem-theorem) | Salem's theorem (1963, Thm IV) in construction form: every Pisot number is a two-sided limit of Salem numbers, with the z^m·P ± P* family and the quadratic-unit Chebyshev family carried as theorems; external sources only |
| PALOMAR-2026-09-02-000014 | [golden-family-trace-forms](https://github.com/stalex444/golden-family-trace-forms) | trace forms of a tensor product of finite free algebras: the tmul identity, the Kronecker Gram matrix, the signed discriminant product, the Sylvester signature product rule; the compositum ℚ(ρ,Q) as the instance (signature (7,5), discriminant (−23)⁴(−283)³) |
| PALOMAR-2026-09-03-000004 | [radial-harmonic-classification](https://github.com/stalex444/radial-harmonic-classification) | the classification of radial harmonic functions on a finite-dimensional real inner product space (every radial harmonic function is an affine function of the Newtonian potential, or of the logarithm in the plane), the radial Laplacian, and the stability of circular orbits for power-law central forces; Ehrenfest's 1917 dimensionality argument as the application |
