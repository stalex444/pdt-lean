...
| **PdtEhrenfest** | **Ehrenfest's 1917 dimensionality argument, as real analysis.** For the effective potential `U(r) = a r⁻² − b r⁻ᵖ` (`a, b > 0`, real exponent `p` via `Real.rpow`; Newtonian `n` dimensions is `p = n − 2`): the critical-point criterion and its uniqueness for `p ≠ 2`; `U''(rstar) = 2a(2 − p) rstar⁻⁴`, whose sign is the sign of `2 − p`; stability for `p < 2` and instability for `p > 2`; **no local minimum at all** for `p > 2`, hence no bounded non-circular motion; and `stable_iff_three` — among `n ≥ 3` a local minimum exists **iff `n = 3`**. Also the logarithmic `n = 2` case and the frequency ratio `U''(rstar)·rstar⁴/(2a) = 2 − p = 4 − n`. Every statement is about real functions of one real variable; the physical reading is not formalized. |
