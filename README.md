...
> **256 declarations · 17 modules · no `sorry`, no `native_decide`.** A mix of substantive results, API wrappers, and disclosed elementary numeral identities — the per-module table below states what each module proves; all kernel-clean over the three standard axioms (`propext`, `Classical.choice`, `Quot.sound`). (169 use the `theorem` keyword; the remaining 87 are `lemma` helpers. Six of the 256 are `private` and so are not addressable by name from an importing file: the audit checks the other 250 directly, and those six transitively, since `#print axioms` reports the axioms of everything a result depends on.) The one-command verifier below builds the project and prints **PASS** with the full axiom trace — or fails loudly.
